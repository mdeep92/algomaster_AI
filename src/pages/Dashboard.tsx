...
export default function Dashboard() {
  const { isCompleted, completed } = useCompletedTopics();
  const masteredCount = completed.length;
  const progressPct = TOPICS.length > 0 ? Math.round((masteredCount / TOPICS.length) * 100) : 0;
        ...
        <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">{masteredCount} / {TOPICS.length}</p>
            </div>
          </div>
            ...
            return (
              ...
              >
                <div className={
                    ...
                    ? "flex-shrink-0 w-12 h-12 bg-emerald-500/10 text-emerald-400 rounded-lg flex items-center justify-center transition-colors"
                    : "flex-shrink-0 w-12 h-12 bg-neutral-900 rounded-lg flex items-center justify-center group-hover:bg-indigo-500/10 group-hover:text-indigo-400 transition-colors text-neutral-500"
                }>
